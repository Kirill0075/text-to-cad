# Prompt: Shared implementation library for the 20 simple CAD prompt scripts.
...
def make_flat_rectangular_plate():
    length = 90.0
    width = 55.0
    thickness = 6.0
    hole_diameter = 6.2
    ...
    with BuildPart() as model:
        ...
        for x_pos in (-34.0, 34.0):
            for y_pos in (-18.5, 18.5):
                _hole_z(hole_diameter, thickness + 2.0, x_pos, y_pos)

    part = _safe_chamfer(model.part, _line_edges(model.part), 1.0)
    return _finish(part, "flat_rectangular_plate_corner_holes_chamfered_edges", Color(0.62, 0.66, 0.70, 1.0))
...
def make_circular_flange_plate():
    outer_radius = 45.0
    thickness = 10.0
    bore_radius = 14.0
    bolt_count = 8
    bolt_radius = 3.0
    bolt_circle_radius = 34.0

    with BuildPart() as model:
        Cylinder(outer_radius, thickness, align=(Align.CENTER, Align.CENTER, Align.MIN))
        _hole_z(bore_radius * 2.0, thickness + 2.0, 0.0, 0.0)
        for index in range(bolt_count):
            x_pos, y_pos = _polar_point(bolt_circle_radius, tau * index / bolt_count)
            _hole_z(bolt_radius * 2.0, thickness + 2.0, x_pos, y_pos)

    edge_set = []
    for z_pos in (0.0, thickness):
        edge_set.extend(_circular_edges(model.part, radius=outer_radius, axis="z", coordinate=z_pos))
        edge_set.extend(_circular_edges(model.part, radius=bore_radius, axis="z", coordinate=z_pos))
    part = _safe_chamfer(model.part, edge_set, 0.8)
    return _finish(part, "circular_flange_plate_central_bore_eight_bolt_holes", Color(0.64, 0.67, 0.71, 1.0))
...
def make_l_bracket():
    ...
    leg_thickness = 7.0
    leg_height = 45.0
    back_center_y = base_width / 2.0 - leg_thickness / 2.0
    ...
    with BuildPart() as model:
        ...
        with Locations(Location((0.0, back_center_y, base_thickness))):
            Box(base_length, leg_thickness, leg_height, align=(Align.CENTER, Align.CENTER, Align.MIN), mode=Mode.ADD)
        for x_pos in (-24.0, 24.0):
            _hole_z(5.5, base_thickness + 2.0, x_pos, -8.0)
            _hole_y(5.5, leg_thickness + 2.0, x_pos, back_center_y, 27.0)

    inside_y = back_center_y - leg_thickness / 2.0
    inside_edges = []
    for edge in _line_edges(model.part):
        bbox = edge.bounding_box()
        if abs(bbox.min.Z - base_thickness) < 0.08 and abs(bbox.max.Z - base_thickness) < 0.08:
            if abs(bbox.center().Y - inside_y) < 0.08 and bbox.size.X > 20.0:
                inside_edges.append(edge)
    part = _safe_fillet(model.part, inside_edges, 2.5)
    part = _safe_chamfer(part, _line_edges(part), 0.4)
    return _finish(part, "simple_l_bracket_four_mounting_holes_filleted_inside_corner", Color(0.55, 0.62, 0.68, 1.0))
...
def make_u_bracket():
    ...
    with BuildPart() as model:
        Box(base_length, base_width, base_thickness, align=(Align.CENTER, Align.CENTER, Align.MIN))
        for x_pos in (-ear_x, ear_x):
            with Locations(Location((x_pos, 0.0, base_thickness))):
                Box(ear_thickness, base_width, ear_height, align=(Align.CENTER, Align.CENTER, Align.MIN), mode=Mode.ADD)
        _hole_x(9.0, base_length + 2.0, 0.0, 0.0, base_thickness + 18.0)

    part = _safe_fillet(model.part, _line_edges(model.part), 0.8)
    return _finish(part, "u_shaped_bracket_parallel_ears_aligned_pin_holes", Color(0.48, 0.56, 0.62, 1.0))
...
def make_rectangular_electronics_housing_base():
    length = 110.0
    width = 75.0
    height = 32.0
    wall = 3.0
    ...
    with BuildPart() as model:
        ...
        with Locations(Location((0.0, 0.0, floor))):
            Box(length - 2.0 * wall, width - 2.0 * wall, height, align=(Align.CENTER, Align.CENTER, Align.MIN), mode=Mode.SUBTRACT)
        for x_pos in (-38.0, 38.0):
            for y_pos in (-23.0, 23.0):
                with Locations(Location((x_pos, y_pos, floor))):
                    Cylinder(5.5, 12.0, align=(Align.CENTER, Align.CENTER, Align.MIN), mode=Mode.ADD)
                with Locations(Location((x_pos, y_pos, floor + 12.0))):
                    Cylinder(1.8, 9.0, align=(Align.CENTER, Align.CENTER, Align.MAX), mode=Mode.SUBTRACT)
        for x_pos in (-48.0, 48.0):
            for y_pos in (-31.0, 31.0):
                _hole_z(4.2, floor + 2.0, x_pos, y_pos)

    part = _safe_fillet(model.part, _line_edges(model.part), 0.7)
    return _finish(part, "rectangular_electronics_housing_base_internal_cavity_bosses_corner_holes", Color(0.44, 0.54, 0.60, 1.0))
