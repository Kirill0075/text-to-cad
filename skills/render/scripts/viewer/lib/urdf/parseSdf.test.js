...
test("parseSdf ignores CAD Explorer input motion plugin channels as static metadata", () => {
  const root = sdfRoot([
    el("model", { name: "robot" }, [
      link("base_link"),
      link("wheel_link", { parent: "base_link" }),
      joint(1, "base_link", "wheel_link", "continuous"),
      el("plugin", { name: "cad_explorer_input_motion", filename: "cad-explorer-input-motion" }, [
...
test("parseSdf ignores CAD Explorer link pose playback tracks as static metadata", () => {
  const root = sdfRoot([
    el("model", { name: "robot" }, [
      link("base_link"),
